...
[Group( "Worst Walker" )]
[Title( "Worst Walker - Player Controller" )]
public sealed class PlayerController : Component
{
	...
	[Property] public GameObject Flashlight;
	[Property] public GameObject NPC;

	[Property] public GameObject go;
	...
	private void UpdateCamera()
	{
		...
		var targetCameraPos = Transform.Position + new Vector3( 0, 0, EyeHeight + 32);
	...
	private void Sosulka()
    {
		bool lox = Input.Released("use");//Released
		bool lox2 = Input.Released("attack2");
		bool lox3 = Input.Released("Flashlight");
		bool lox4 = Input.Released("Drop");
		bool lox5 = Input.Released("Reload");
		...
		if (lox)
		{
			...
			//go.Clone(result.EndPosition + result.Normal * 10);
		}
		if (lox2)
        {
			var result = Scene.Trace.Ray(startPos, startPos + (dir * maxDist))
		...
		if (lox4)
		{
			...
			if (result.Hit)
			{
			NPC.Clone(result.EndPosition + result.Normal * 10);
			NPC.NetworkSpawn();
			}
		}
